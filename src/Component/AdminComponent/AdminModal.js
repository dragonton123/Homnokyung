...
class AdminModal extends Component {
    ...
    comfirm_payment = async () => {
        const object = {
            ...
            user_id :  this.props.element.user_id,
            land_id : this.props.element.land_id
        }
        console.log(object)


        try{
            await post(object,'order/admin_confirm_payment',sessionStorage.getItem('user_token')).then((res)=>{
                if(res.success){
                    swal(res.message, {
                        icon: "success",
                        buttons: false,
                ...
                <div >
                    <div style={{ fontSize:30,marginRight:100,marginLeft:100}}>
                        ...
                        <div style={{ marginLeft:50 ,display:"flex",flexDirection:"row",justifyContent:"center"}}>
                            {payment_status === 1?<button type="button" style={{marginRight:50}} className="default_button" onClick={()=>{this.comfirm_payment()}}>ยืนยันรายการ</button>:null}
